...
export function PageTagline() {
  ...
  const m = MODULES[idx]
  return (
    <div className="mb-2.5 sm:mb-4 flex flex-wrap items-baseline gap-x-3 gap-y-0.5">
      <h2 className="font-display font-bold text-[19px] sm:text-[22px] tracking-tight leading-none flex items-baseline gap-2">
        <span className="text-gold text-base leading-none">✦</span>
        <span className="font-sans font-medium text-[13px] sm:text-[14px] text-muted">{m.group}<span className="mx-1.5 opacity-70">/</span></span>
        {m.tab}
      </h2>
      {/* the explainer is desktop-only — on a phone it costs a line and says what the page shows anyway */}
      <span className="hidden sm:inline text-sm text-muted">{m.tagline}</span>
    </div>
  )
}
...
export function JourneyHome() {
  ...
  if (!open) {
    return (
      <button onClick={() => setOpen(true)} className="mb-4 inline-block py-2 min-h-[32px] text-xs text-muted hover:text-ink underline decoration-dotted transition-colors">
  ...
  return (
    <div className="card p-4 mb-5 animate-fadeUp">
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        <button onClick={hide} title="Hide" className="text-muted hover:text-ink text-xl leading-none shrink-0 -m-1 p-1 min-w-[32px] min-h-[32px] grid place-items-center">×</button>
      </div>
...
export function JourneyNav() {
  ...
  return (
    <div className="mt-6 sm:mt-8 pt-4 sm:pt-5 border-t border-white/[0.07] flex items-stretch justify-between gap-3">
      {prev ? (
        <button onClick={() => nav(prev.to)} className="group flex flex-col items-start px-4 py-2.5 rounded-xl border border-white/10 bg-white/[0.02] hover:border-white/25 transition-colors text-left">
          <span className="kicker text-muted">← Previous</span>
      ...
      {next ? (
        <button onClick={() => nav(next.to)} className="group flex flex-col items-end px-4 py-2.5 rounded-xl border border-gold/30 bg-gold/[0.06] hover:bg-gold/[0.12] hover:border-gold/50 transition-colors text-right max-w-md">
          <span className="kicker text-gold">Next step →</span>
        ...
        <button onClick={() => nav('/')} className="flex flex-col items-end px-4 py-2.5 rounded-xl border border-white/10 bg-white/[0.02] hover:border-white/25 transition-colors text-right">
          <span className="kicker text-muted">End of the journey</span>
