...
**Access is INVITE-ONLY — Google sign-in is ON** (2026-08-25, owner request). `AUTH_ENABLED = true` in `src/lib/config.ts` is the master switch. Google sign-in + a Firestore allow-list, permanent super-admin `sai.prasanth@themindshare.in` (hardcoded in `auth.tsx` AND `firestore.rules` — change both together), admin-only **Manage users** panel at `/admin` reached from the header account menu, writing live to the `allowed_users` collection with no redeploy. Firebase project **verdix-d369c**; public web config in gitignored `.env.local`, which `.dockerignore` deliberately does NOT exclude so `fly deploy` inlines it at build.

⚠ **PENDING CONSOLE STEP — sign-in cannot succeed until this is done:** `verdix-elections.fly.dev` is **not** in Firebase → Authentication → Settings → Authorized domains. As of 2026-08-25 that list holds only `localhost`, `verdix-d369c.firebaseapp.com`, `verdix-d369c.web.app` and the stale `verdix-elections.netlify.app`. Until the Fly domain is added, every sign-in fails `auth/unauthorized-domain` (the login screen says so, naming the hostname). Check it any time with:
`curl -s "https://identitytoolkit.googleapis.com/v1/projects?key=<VITE_FIREBASE_API_KEY>"` → `authorizedDomains`.

⚠ Sign-in uses `signInWithPopup` — a full-page `signInWithRedirect` was tried and **broke** sign-in; don't "fix" it back.
⚠ **The CSP must allow `https://apis.google.com` in `script-src`.** Firebase Auth loads `apis.google.com/js/api.js` for its iframe manager even on the v9 modular SDK; with `script-src 'self'` alone the browser refuses it, no popup opens, and the sign-in button silently does nothing. This was caught by reading console for CSP violations **on the deployed site** — the local `npm run preview` serves no CSP at all, so it cannot catch this class of bug.
**To turn access back OFF:** set `AUTH_ENABLED = false`; the Caddyfile's Google/Firebase CSP entries can then be dropped.
...
- **Full-screen / focus mode** (`components/FocusMode.tsx` + `useFocusStore`): the header button hides the app **header** (wordmark, theme toggle, account) AND calls `requestFullscreen()` so the browser drops its tab strip and address bar. ⚠ **The left rail and the phone tab bar STAY** — they are navigation, not chrome; hiding them was tried on 08-25 and the owner rejected it ("in full screen the left side ones are not coming"). The FilterBar stays too, being the control surface, with `pr-12` to clear the fixed exit button over its right end. ⚠ Focus and fullscreen are **decoupled on purpose** — `requestFullscreen` needs a user gesture and can be refused, and when it is, hiding the header is still most of the win. Esc is handled **in the app as well as by the browser**, because a refused fullscreen leaves nothing for the browser's own Esc to exit. `fullscreenchange` syncs the flag back whenever fullscreen ends by any route (Esc, F11, the OS).
...
### Map
`ChoroplethMap` — symbol-layer **labels**: state names come from a **dedicated point source** (`state-points`), ONE feature per state — ⚠ **never drive the state-label layer off the polygon source**: MapLibre places a label on *every part* of a MultiPolygon, and Andaman & Nicobar is 142 islands, so the map asked for 269 placements across the 36 states and collision-hiding left its name visible four times in open sea (owner, 2026-08-25). `stateLabelGeo` (a `useMemo` in the component) reduces each state to its **largest part**, then takes that part's area centroid — falling back to a grid-search for the interior point furthest from any edge when the centroid lands outside the ring, which it does for Andaman & Nicobar and Dadra & Nagar Haveli. Verified: 36 points, 0 outside their own state. Constituency names (`AC_NAME` / `pc_name`, zoom ≥ 4.5) still come from the seat polygons with `text-allow-overlap: false` so overlaps auto-hide. **Self-hosted Open Sans glyphs** in `public/glyphs` (the style needs a `glyphs` URL or text won't render at all). No fullscreen (ResizeObserver sizing). Winner / Alliance / Safe-vs-Swing colour modes; behavioural pan/zoom fence. **Join rules and the camera-regression warnings are in [CONVENTIONS.md](CONVENTIONS.md) — read them before touching the map.**
...
- **States page order (owner-set 2026-08-24):** the **seat map + swing + strongholds lead the page**, followed by the rest of the analysis cards (vote share + seats, who-beats-whom, turnout, reservation, close seats). Then the **scorecard, standalone and full-width** — since 08-25 it carries **EVERY election the state has held** (one Seats+Vote% column pair each, assemblies then Lok Sabhas, newest first) with the election selected above **highlighted in gold**, so the `max-w-3xl` cap is gone: it now earns the width and scrolls inside its own card. **Each election is drawn as its own BOX** — a left/right rule down its two columns, closed top and bottom, on a faint panel tint — so the eye groups Seats+Vote% as one election instead of reading ten loose numeric columns. Boxes are per-cell borders, because the table is `border-separate` with spacing 0; the gap between boxes is the edge cells' padding. **Everything is centre-aligned** except the sticky Party column. Group headers are `colSpan={2}`, centre- and middle-aligned. The **LS − AE** gap stays anchored on the LATEST of each arena — a 2012 assembly against a 2024 Lok Sabha would mean nothing. Last, the **winner matrix and the positions table, each FULL WIDTH, stacked one above the other**. ⚠ Both earlier pairings are **superseded** — first scorecard+positions, then matrix+positions (owner, 2026-08-25: "these are not good side by side"). Neither belongs in a half-width column: they are wide, horizontally-scrolling tables, and side by side each showed about four of its columns with the rest behind an inner scrollbar. Full width, the matrix needs no inner scroll at all up to ~1900px and the positions table shows 1638 of its 1666px. The matrix's `<table>` carries `w-full` so its columns spread to fill a card wider than they need (a state with few elections used to leave half the card empty); once the columns need more room their `min-w-[104px]` wins and it scrolls as before.
