...
- **States page order (owner-set 2026-08-24):** the **seat map + swing + strongholds lead the page**, followed by the rest of the analysis cards (vote share + seats, who-beats-whom, turnout, reservation, close seats). Then the **scorecard, standalone and full-width** — its table is capped at `max-w-3xl` because six columns spread across 1550px turned into mostly whitespace. Last, the two wide read-outs share a row: **winner matrix beside the positions table** (`grid xl:grid-cols-2`, equal height — `ChartCard` is `flex flex-col`); each still scrolls inside its own card at half width. ⚠ The earlier scorecard+positions pairing is **superseded** — don't restore it.
