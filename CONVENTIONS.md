...
- **Verifying contrast headlessly: composite onto `CARD_SURFACE`, never onto "whatever opaque ancestor you find".** `body` and `.card` are gradient-only and report `rgba(0,0,0,0)`, so a naive walk up the tree finds no opaque layer and silently drops the last element's alpha — an 11% tint then measures as a full-saturation fill and the checker invents ~150 failures that do not exist. Terminate the walk on `light [244,245,240] / dark [12,34,36]` (the `CARD_SURFACE` values in colors.ts), which is the exact surface `inkOn()` blends against, so the checker and the code agree on the reference.
- **Alpha is not a free contrast dial — there is a dead band.** For party colour used as a fill, alpha ~0.45–0.70 is where `max(black, white)` ink bottoms out (blended luminance ≈ 0.179 → 4.58:1) and where the chosen ink starts *flipping between themes*, so the same cell reads black-on-pink in light and white-on-pink in dark. Either go opaque (alpha 1 — no blend at all, ink identical in both themes for all 49 palette hexes, floor 4.60:1) or stay at/below ~0.34. Don't split the difference.
